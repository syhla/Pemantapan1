...
<div class="card" style="padding:28px">
    ...
    <form action="{{ route('admin.gerai.update', $gerai->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="nama_gerai" value="{{ old('nama_gerai', $gerai->nama_gerai) }}">
        </div>

        <div class="form-group">
            <label>Email Login</label>
            <input type="email" name="email" value="{{ old('email', $gerai->user->email ?? '') }}">
        </div>

        <div class="form-group">
            <label>Alamat</label>
            <input type="text" name="alamat" value="{{ old('alamat', $gerai->alamat) }}">
        </div>

        <div class="form-group">
            <label>Kota</label>
            <input type="text" name="kota" value="{{ old('kota', $gerai->kota) }}">
        </div>

        <div class="form-group">
            <label>Telepon</label>
            <input type="text" name="telepon" value="{{ old('telepon', $gerai->telepon) }}">
        </div>
