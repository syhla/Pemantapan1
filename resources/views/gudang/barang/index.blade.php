...
<div style="max-width:1200px; margin:auto; padding:20px;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        ...
        <div style="display:flex;gap:10px;align-items:center">
            ...
            <!-- 🔍 SEARCH -->
            <form method="GET" action="{{ route(request()->route()->getName()) }}" style="display:flex;align-items:center;gap:8px">
                ...
                <tr style="transition:0.3s;" 
                    ...
                    <td style="padding:12px;border-bottom:1px solid #e5e7eb;text-align:center;">
                        ...
                        @elseif($b->status == 'pending_edit')
                            <span style="background:#fef9c3;color:#854d0e;padding:4px 10px;border-radius:8px;font-size:13px;">⏳ Pending</span>
                        @elseif($b->status == 'pending_delete')
                            <span style="background:#fee2e2;color:#991b1b;padding:4px 10px;border-radius:8px;font-size:13px;">🗑️ Delete</span>
                        @elseif($b->status == 'rejected')
                            <span style="background:#f3f4f6;color:#6b7280;padding:4px 10px;border-radius:8px;font-size:13px;">
                                ❌ Rejected
                            </span>
                        @endif
                    </td>
                    ...
                    <td style="padding:12px;border-bottom:1px solid #e5e7eb;display:flex;gap:5px;justify-content:center;flex-wrap:wrap;">
                        @if(in_array($b->status, ['active','rejected']))
                            <a href="{{ route('gudang.barang.edit', $b->id) }}" 
                            ...
                            <form action="{{ route('gudang.barang.destroy', $b->id) }}" method="POST" onsubmit="return confirm('Yakin mau hapus?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" style="padding:5px 10px;font-size:13px;">
                                    🗑️ Delete
                                </button>
                            </form>
                        @else
